...
#if 0		/* 016.05.26 */
static void __iomem	*ckc_base = NULL; //(void*)HwCKC_BASE;
static void __iomem	*pmu_base = NULL; //(void*)HwPMU_BASE;
static void __iomem	*membus_cfg_base = NULL; //(void*)HwMBUSCFG_BASE;
static void __iomem	*ddibus_cfg_base = NULL; //(void*)HwDDI_CONFIG_BASE;
static void __iomem	*gpubus_cfg_base = NULL; //(void*)HwGRPBUSCONFIG_BASE;
static void __iomem	*iobus_cfg_base = NULL; //(void*)HwIOBUSCFG_BASE;
static void __iomem	*vpubus_cfg_base = NULL; //(void*)HwVIDEOBUSCONFIG_BASE;
static void __iomem	*hsiobus_cfg_base = NULL; //(void*)HwHSIOBUSCFG_BASE;
#else
static void __iomem	*ckc_base = (void*)HwCKC_BASE;
static void __iomem	*pmu_base = (void*)HwPMU_BASE;
static void __iomem	*membus_cfg_base = (void*)HwMBUSCFG_BASE;
static void __iomem	*ddibus_cfg_base = (void*)HwDDI_CONFIG_BASE;
static void __iomem	*gpubus_cfg_base = (void*)HwGRPBUSCONFIG_BASE;
static void __iomem	*iobus_cfg_base = (void*)HwIOBUSCFG_BASE;
static void __iomem	*vpubus_cfg_base = (void*)HwVIDEOBUSCONFIG_BASE;
static void __iomem	*hsiobus_cfg_base = (void*)HwHSIOBUSCFG_BASE;
...
static inline int tcc_find_clkctrl(tCLKCTRL *CLKCTRL)
{
	...
	if (CLKCTRL->freq <= (XIN_CLK_RATE/2)) {
		...
		for (i=0 ; i<MAX_CLK_SRC ; i++) {
			if (gd->arch.tcc.stClockSource[i] == 0)
				continue;
			div[i] = (gd->arch.tcc.stClockSource[i]+CLKCTRL->freq-1)/CLKCTRL->freq;
			if (div[i] > (CLKCTRL_CONFIG_MAX+1))
				div[i] = CLKCTRL_CONFIG_MAX+1;
			else if (div[i] < (CLKCTRL_CONFIG_MIN+1))
				div[i] = CLKCTRL_CONFIG_MIN+1;
			clk_rate = gd->arch.tcc.stClockSource[i]/div[i];
		...
		CLKCTRL->freq = gd->arch.tcc.stClockSource[searchsrc]/div[searchsrc];
...
static inline int tcc_find_clkctrl_for_cpu1(tCLKCTRL *CLKCTRL)
{
	...
	for (i=0 ; i<MAX_CLK_SRC ; i++) {
		if (gd->arch.tcc.stClockSource[i] == 0)
			continue;
		unit = gd->arch.tcc.stClockSource[i]/(CLKCTRL_CONFIG_MAX+1);
	...
	CLKCTRL->freq = div[searchsrc]*(gd->arch.tcc.stClockSource[searchsrc]/(CLKCTRL_CONFIG_MAX+1));
	CLKCTRL->config = 0;
...
static inline int tcc_find_pclk(tPCLKCTRL *PCLKCTRL, tPCLKTYPE type)
{
	...
	for (i=(MAX_CLK_SRC-1) ; i>=0 ; i--) {
...
#endif
		{
			if (gd->arch.tcc.stClockSource[i] == 0)
				continue;
			if ((gd->arch.tcc.stClockSource[i] >= PCLKCTRL_MAX_FCKS) && (type == PCLKCTRL_TYPE_XXX))
				continue;
		}
		...
		if (type == PCLKCTRL_TYPE_XXX)
			...
			err_dco = tcc_ckc_pclk_dco(PCLKCTRL, &div_dco, gd->arch.tcc.stClockSource[i], PCLKCTRL_DIV_DCO_MIN, div_max);

		/* divider mode */
		err_div = tcc_ckc_pclk_divider(PCLKCTRL, &div_div, gd->arch.tcc.stClockSource[i], PCLKCTRL_DIV_MIN+1, div_max+1);
	...
	if (PCLKCTRL->md == PCLKCTRL_MODE_DCO) {
		...
		if (PCLKCTRL->div > div_max/2)
			u64_tmp = (unsigned long long)gd->arch.tcc.stClockSource[searchsrc]*(unsigned long long)(div_max-PCLKCTRL->div);
		else
			u64_tmp = (unsigned long long)gd->arch.tcc.stClockSource[searchsrc]*(unsigned long long)PCLKCTRL->div;
		...
		if (PCLKCTRL->div >= (PCLKCTRL_DIV_MIN+1) && PCLKCTRL->div <= (div_max+1))
			...
			return -1;
		PCLKCTRL->freq = gd->arch.tcc.stClockSource[searchsrc]/(PCLKCTRL->div+1);
	}
	return 0;
}
...
static inline void tcc_ckc_reset_clock_source(int id)
{
	...
	case CPU0_SRC_CH:
	case MEM_SRC_CH:
		gd->arch.tcc.stClockSource[id] = 0;
		gd->arch.tcc.stClockSource[MAX_TCC_PLL+id] = 0;
		return;
	}
	...
	if (id < MAX_TCC_PLL) {
		gd->arch.tcc.stClockSource[id] = tcc_ckc_pll_get_rate(id);
		gd->arch.tcc.stClockSource[MAX_TCC_PLL+id] = tcc_ckc_plldiv_get_rate(id);
	}
}
...
void tcc_ckc_reset_ops(void)
{
	tcc_ckc_set_ops((struct tcc_ckc_ops *)&tcc897x_ops);
}
...
void tcc_ckc_init(void)
{
	...
	gd->arch.tcc.stClockSource[MAX_TCC_PLL*2] = XIN_CLK_RATE;	// XIN
	gd->arch.tcc.stClockSource[MAX_TCC_PLL*2+1] = XTIN_CLK_RATE;	// XTIN
	...
	tcc_set_clkctrl(FBUS_IO, ENABLE, XIN_CLK_RATE/2);
	tcc_set_clkctrl(FBUS_SMU, ENABLE, XIN_CLK_RATE/2);
	/* set clkctrl */
#if !defined(CONFIG_USB_UPDATE)
	for (i=0 ; i<FBUS_MAX ; i++) {
#if defined(TSBM_ENABLE)
		if (i == FBUS_CPU0 || i == FBUS_MEM || i == FBUS_HSIO)
			continue;
#else
