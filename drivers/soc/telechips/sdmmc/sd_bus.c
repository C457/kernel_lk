...
static unsigned long SD_BUS_Scan(PSD_SLOT_T pSlot)
{
	...
	{
		debug("[mmc] eMMC card is applied to high speed mode.\n");
		...
		if (SD_HW_IS_DDR_MODE( SD_BUS_GetBootSlotIndex() ) &&
				pSlot->bw == 8) {
			debug("[mmc] eMMC card BUS_WIDTH is set to 8 bit dual data rate.\n");
			SD_SLOT_switchEXTCSD(pSlot,183,6,0);
		} else if (SD_HW_IS_DDR_MODE( SD_BUS_GetBootSlotIndex() ) &&
				pSlot->bw == 4) {
			debug("[mmc] eMMC card BUS_WIDTH is set to 4 bit dual data rate.\n");
			SD_SLOT_switchEXTCSD(pSlot,183,5,0);
		} else if (!SD_HW_IS_DDR_MODE( SD_BUS_GetBootSlotIndex() ) &&
				pSlot->bw == 8) {
			debug("[mmc] eMMC card BUS_WIDTH is set to 8 bit single data rate.\n");
			SD_SLOT_switchEXTCSD(pSlot,183,2,0);
		} else if (!SD_HW_IS_DDR_MODE( SD_BUS_GetBootSlotIndex() ) &&
				pSlot->bw == 4) {
			debug("[mmc] eMMC card BUS_WIDTH is set to 4 bit single data rate.\n");
			SD_SLOT_switchEXTCSD(pSlot,183,1,0);
		}
	} else if( (pSlot->id == MMC_CARD_SD) &&
			SD_HW_IS_HIGH_SPEED( SD_BUS_GetBootSlotIndex() ) ) {
		debug("[MMC] SD card is applied to high speed mode.\n");
...
//	printf("pSlot: %d, cd: %d\n",pSlot->iIndex,  SD_SLOT_IsCardDetected(pSlot));
...
static int do_emmc_erase(PSD_SLOT_T pSlot, unsigned long start_addr, unsigned long erase_size,
		    int low_format)
{
        debug("[SD/MMC erase] start_addr : %ld(sector)\n", start_addr);
        debug("[SD/MMC erase] start_addr + erase_size : %ld(sector)\n", start_addr+erase_size);
	...
	debug("Waiting erase complete response..");
	do{
		if( (i++ % 50000) == 0 )
			debug(".");
...
int erase_emmc_bootpartition(SD_SLOT_T *mmc, int slot_id)
{
	...
	while( i++ < 2 )
	{
		...
		if( do_emmc_erase(mmc, 0, boot_partition_size, 1) ) {
			debug("Erasing boot partition %d fail...\n", i);
