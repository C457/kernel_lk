...
int fwdn_mmc_config_write( unsigned int rom_size,  // ks_8930
...
int fwdn_mmc_update_secure_bootloader(unsigned int rom_size, int boot_partition)
{
	...
	BOOTSD_Header_Info_T bootsd_h;
	NSK_Header_Info_T nsk_h;
	memset(&bootsd_h, 0x00, sizeof(BOOTSD_Header_Info_T));
	memset(&nsk_h, 0x00, sizeof(NSK_Header_Info_T));
#endif // TCC_NSK_ENABLE
...
//////////////////////////////////////////////
// 1. Calculate nsk and boot header         //
// 2. Write boot header                     //
// 3. Write NSK header                      //
// 4. Write config code(DRAM init code)     //
// 4. Write CM4 code                        //
// 5. Write CA7A code                       //
// 6. Write CA7 code                        //
// 7. Write boot loader                     //
//////////////////////////////////////////////
	...
	printf("[NSK] calculate & write boot partition no.%d start!!\n", boot_partition);
	...
	if(calculate_nsk_boot_header(&secure_h, &bootsd_h, &nsk_h, boot_partition, 1/*fwdn_mode*/))
	{
		printf("[NSK] calculate nsk boot header failed!!\n");
		return -1;
	}

	if(mmc_header_write_nsk(&bootsd_h, boot_partition))
	{
		printf("[NSK] write boot header failed!!\n");
		return -1;
	}

	if(mmc_nsk_header_write_nsk(&nsk_h, bootsd_h.ulNSKHeaderBase, boot_partition))
	{
		printf("[NSK] write nsk boot header failed!!\n");
		return -1;
	}

	if(mmc_config_write_nsk(bootsd_h.ulConfigCodeBase, secure_h.dram_init_size, (unsigned char*)(buf+(unsigned int)secure_h.dram_init_start), boot_partition))
	{
		printf("[NSK] write config code failed!!\n");
		return -1;
	}

	if(mmc_cm4_write_nsk(nsk_h.ulCM4CodeBase, nsk_h.ulCM4CodeSize, (unsigned char*)(buf+(unsigned int)secure_h.cm4_start), boot_partition))
	{
		printf("[NSK] write cm4 failed!!\n");
		return -1;
	}

	if(mmc_ca7a_write_nsk(nsk_h.ulCA7ACodeBase, nsk_h.ulCA7ACodeSize, (unsigned char*)(buf + (unsigned int)secure_h.ca7in_start), boot_partition))
	{
		printf("[NSK] write ca7a failed!!\n");
		return -1;
	}

	if(mmc_ca7_write_nsk(nsk_h.ulCA7CodeBase, nsk_h.ulCA7CodeSize, (unsigned char*)(buf + (unsigned int)secure_h.ca7ext_start), boot_partition))
	{
		printf("[NSK] write ca7 failed!!\n");
		return -1;
	}

	if(mmc_rom_write_nsk(bootsd_h.ulBootloaderBase, bootsd_h.ulROMFileSize, (unsigned char*)(buf + (unsigned int)secure_h.bootloader_start), boot_partition))
	{
		printf("[NSK] write boot rom failed!!\n");
		return -1;
	}
#endif // !defined(TCC_NSK_ENABLE)
