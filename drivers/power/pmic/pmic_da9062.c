...
int da9062_init(unsigned char bus)
{
	static const char name[] = "da9062";
	...
	p->hw.i2c.addr = SLAVE_ADDR_DA9062;
	p->hw.i2c.tx_num = 1;
	p->bus = bus;
	...
	printf( "CFG1:0x%x, CFG2:0x%x, CFG3:0x%x, CFG4:0x%x\n",
...
{
	unsigned char reg = 0, old_value, value, mask;
...
static int da9062_buck_set_voltage(da9062_src_id id, unsigned int mV)
{
	...
	if (mV > max || mV < min){
		printf("denied DA9062_BUCK%d value set!\n", id);
		return -1;
	}
	...
	printf( "%s: id:%d, mV:%d, reg:0x%x, value:0x%x\n", __func__, id, mV, reg, value);
	da9062_write(reg, value);
