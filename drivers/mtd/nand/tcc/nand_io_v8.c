...
#define	HwCHCTRL_EN_EN				Hw0					// DMA channel is enabled. If software type transfer is selected, this bit generates DMA request directly, or if hardware type transfer is used, the selected interrupt request flag generate DMA request
	...
	#define NAND_DMA_BASE_ADDRESS	(HwGDMA0_BASE)
		...
		#endif
	#elif defined(TCC88XX) || defined(TCC892X) || defined(TCC8925S) || defined(TCC893X) || defined(TCC889X) || defined(TCC896X) || defined(TCC897X)
		#define GPIO_BASE_ADDRESS		(HwGPIO_BASE)
		#define EDI_BASE_ADDRESS		(HwEDI_BASE)
		#define NFC_BASE_ADDRESS		(HwNFC_BASE)
		#define ECC_BASE_ADDRESS		(HwECC_BASE)
		...
		#define PIC_BASE_ADDRESS		(HwPIC_BASE)
			...
			#define NAND_DMA_BASE_ADDRESS	(HwGDMA2_BASE)
		#endif
	#elif defined(TCC801X) || defined(TCC370X) || defined(TCC371X) || defined(TCC800X)
		#define NFC_BASE_ADDRESS		(HwNFC_BASE)
...
#define BL1_MEDIA_TYPE_SPARE_6BIT					0x00300000		// Change SPARE ECC to 6Bit(because of spare size overflow)
...
#elif defined(_LINUX_) || defined(NAND_FOR_UBOOT)
...
#if defined(_LINUX_)
	#if defined(NAND_FOR_KERNEL) || defined(NAND_FOR_UBOOT)
		struct dma_buf
		{
			void *v_addr;
...
#if !(defined(TCC801X) || defined(TCC370X) || defined(TCC371X) || defined(TCC800X))
static PPIC s_pPIC;
#endif
...
// POINTER OF NANDFLASH INFOMATION
	{
		...
		( NAND_IO_FEATURE * )WINBOND_NAND_DevInfo,
	}
};
...
#if defined(TCC893X) || defined(TCC897X)
...
#endif
static NAND_IO_ERROR_T NAND_IO_ReadParam_ONFI(NAND_IO_DEVINFO *pstDevInfo);
...
static unsigned int NAND_IO_GetECCMode( unsigned short nEncDec, unsigned short nEccType )
{
	...
	{		
		...
		else if( nEccType == ECC_TYPE_6BIT )
			uiECCMode		= 	ECC_CTRL_ECC_EN_MLC_ECC6_DECODE;
	...
	else if( nEncDec == ECC_ENCODE )
	{
		if( nEccType == ECC_TYPE_4BIT )
			...
			uiECCMode	= 	ECC_CTRL_ECC_EN_MLC_ECC6_ENCODE;
		else if( nEccType == ECC_TYPE_8BIT )
...
static const NAND_IO_ECC_INFO *NAND_IO_GetECCInfo( unsigned short usEccType )
{
	...
	if( usEccType == ECC_TYPE_6BIT )
		return &s_MLC_ECC_6Bit;
#endif
...
static void NAND_IO_CHANGE_SPARE_ECC_TYPE(NAND_IO_DEVINFO *nDevInfo, unsigned int uiEccMode)
{
	...
	switch( uiEccMode )
	{
		...
		case A_ECC_SPARE_6BIT:	// change ECC to 6bit
			nDevInfo->EccType = ECC_TYPE_6BIT;
			break;
		#endif
...
#if defined(NAND_FOR_UBOOT)
unsigned int NAND_IO_IsReady( void )
{
	return NAND_IO_CheckReadyAndBusyStatus();
}
#endif
...
{
#if defined(_WINCE_)
	#if defined(TCC893X) || defined(TCC889X) || defined(TCC896X)
	...
	#if defined(NAND_FOR_UBOOT)
		dma_t.buf_size = 16384; //for Uboot
		dma_t.v_addr = dma_alloc_coherent((unsigned int)dma_t.buf_size, (unsigned long *)dma_t.dma_addr); //for Uboot
	#endif
...
void NAND_IO_Init( void )
{
	...
	memset( &s_pDevInfo, 0x00, sizeof( NAND_IO_DEVINFO ) );
		...
		#if defined(_WINCE_)
			#if defined(TCC893X) || defined(TCC889X) || defined(TCC896X) || defined(TCC897X)
		...
		#if defined(TCC801X) || defined(TCC370X) || defined(TCC371X)
...
*
**************************************************************************/
static unsigned char sNAND_IO_ReadRetryCount[16];
static unsigned char sNAND_IO_ReadRetryRegOfOTP[1024];

unsigned int NAND_IO_Hynix_VerifyReadRetryParameter( const unsigned char *pucParameter, const unsigned char*pucRevParameter, unsigned int uiSize )
{
	unsigned int res = TRUE, i;
...
static void NAND_IO_Hynix_GetReadRetryParameter_For_20nm(NAND_IO_DEVINFO *pDevInfo)
{
...

	{
		...
		for( i = 0; i < 8; i++ )
		{
			res = NAND_IO_Hynix_VerifyReadRetryParameter( &sNAND_IO_ReadRetryRegOfOTP[i*128]
														 ,&sNAND_IO_ReadRetryRegOfOTP[(i*128)+64], 64 );
...
static NAND_IO_ERROR_T NAND_IO_Hynix_GetReadRetryParameter_For_16nm(NAND_IO_DEVINFO *pDevInfo)
{
	NAND_IO_ERROR_T res = NAND_IO_SUCCESS;
	unsigned int i;
	//unsigned int uiReadParameterPos;
	...
	s_pNFC->NFC_SADDR        	= 0x38383838;

	s_pNFC->NFC_SDATA			= 0x52525252;
	...
	for( i = 0; i < 16; i++ )
		sNAND_IO_ReadRetryCount[i] 	= s_pNFC->NFC_SDATA;

	for( i = 0; i < READRETRY_TABLE_SIZE_HYNIX_16NM; i++ )
		sNAND_IO_ReadRetryRegOfOTP[i] = s_pNFC->NFC_SDATA;	
	...
	{
		#if 0
		for( i = 0; i < HYNIX_CIRCULAR_VALUE_MAX_FOR_16NM+1; i++ )
		{
			unsigned int uiReadParameterPos = i * 4;
			ND_TRACE("Param.(%d) : 0x%x\t0x%x\t0x%x\t0x%x\n", i, sNAND_IO_ReadRetryRegOfOTP[uiReadParameterPos]
															   , sNAND_IO_ReadRetryRegOfOTP[uiReadParameterPos+1]
															   , sNAND_IO_ReadRetryRegOfOTP[uiReadParameterPos+2]
															   , sNAND_IO_ReadRetryRegOfOTP[uiReadParameterPos+3]);
		}
		...
		for( i = 0; i < 8; i++ )
		{
			if( NAND_IO_Hynix_VerifyReadRetryParameter( &sNAND_IO_ReadRetryRegOfOTP[i*64], &sNAND_IO_ReadRetryRegOfOTP[(i*64)+32], 32 ) == TRUE )
			{
				//ND_TRACE(" Set Retry Parameter : %d \n", i );
				if( i != 0 )
					memcpy( (void*)sNAND_IO_ReadRetryRegOfOTP, (void*)&sNAND_IO_ReadRetryRegOfOTP[i*64], 32 );
				break;
			}
			else
			{
		...
		if ( i >= 8 )
		{
			//ND_TRACE("All Read-Retry Parameter Sets are Broken!!!\n");
			res = NAND_IO_ERROR_FAIL_TO_SET_RRT;
		}
	}
...
/**************************************************************************
*  DESCRIPTION : 0 ~ n까지 Circular Iteration value 를 위한 Func.
*  				 
*  INPUT : 
*  
*  OUTPUT : 
...
static unsigned int NAND_IO_Hynix_GetCircularValue( unsigned int uiCircularMaxValue )
{
	...
	{
		uiCircularValue++;
		/* Retry Trial : #0 ~ #n (Circular Iteration) */
...
static unsigned char sNAND_IO_ReadRetryAddr_For_Ddie[4] = {0x38, 0x39, 0x3A, 0x3B};
...
static unsigned int NAND_IO_Hynix_SetReadRetry_For_20nm(NAND_IO_DEVINFO *pDevInfo, unsigned int uiStep)
{
	...
	if ( res == TRUE)
	{
		...
		else
		{
			...
			NAND_IO_Hynix_GetCircularValue(NULL_U32);
...
/**************************************************************************
*  DESCRIPTION : Read Retry 레벨 변경 (for Hynix)
*  				 
*  INPUT : 
*  
*  OUTPUT : 
...
*  REMARK : max 12회. 
**************************************************************************/
static unsigned int NAND_IO_Hynix_SetReadRetry_For_16nm(NAND_IO_DEVINFO *pDevInfo, unsigned int uiStep)
{
	unsigned int res = TRUE;
	unsigned int uiMode, uiReadParameterPos;
	unsigned char *pReadRetryAddr=NULL;
	...
	pReadRetryAddr = sNAND_IO_ReadRetryAddr_For_Ddie;

	if( res == TRUE )
	{
		/* Read Retry Flag Setting */
		if( 1 <= uiStep && uiStep <= (HYNIX_CIRCULAR_VALUE_MAX_FOR_16NM + 1) )
		{
			pDevInfo->fReadRetryNow = TRUE;

			uiMode = NAND_IO_Hynix_GetCircularValue( HYNIX_CIRCULAR_VALUE_MAX_FOR_16NM );
			uiReadParameterPos = uiMode * 4;
...
NAND_IO_ERROR_T NAND_IO_GetDeviceInfo( unsigned short nChipNo, NAND_IO_DEVINFO *nDevInfo )
{
	...
	//=====================================================================
	for( j = 0; j < 3000; ++j )	/* Check Read ID during 3 turn */
	{
		#if defined(NFC_VER_50)
	...
	if( bFindMedia == TRUE )
	{
		...
		#if defined(STABILITY_ORIENTED)
		nDevInfo->Feature.UsableFunc &= (~F_MP);			// non-MP operation
		#endif
...
//#endif
	...
	else if( A_READ_RETRY_TYPE_OF( nDevInfo->Feature.MediaType ) == A_RR_H_16NM )
	{
		nDevInfo->fnSetReadRetry = NAND_IO_Hynix_SetReadRetry_For_16nm;
		res = NAND_IO_Hynix_GetReadRetryParameter_For_16nm( nDevInfo );
	}
	...
	if(res ==  NAND_IO_SUCCESS)
	{
		res = NAND_IO_ReadParam_ONFI(nDevInfo);
		if(res == NAND_IO_NOT_ONFI)
			res = NAND_IO_SUCCESS;
	}
...
unsigned int NAND_IO_GetFeatureOfNAND( TNFTL_OBJECT_T *pTnftlObj )
{
...
#if defined(NAND_FOR_UBOOT)
	pTnftlObj->uiPagesPerBlockShift = sNAND_IO_DeviceInfo.ShiftPpB;;
#endif
...
#if defined(TCC8925S)|| defined(TCC893X) || defined(TCC889X) || defined(TCC896X) || defined(TCC897X)
		pTnftlObj->fUseLowerPageOnlyForBL1 = TRUE;
#elif defined(TCC801X) || defined(TCC88XX) || defined(TCC892X) || defined(TCC370X) || defined(TCC371X) || defined(TCC800X)
	...
	#if defined(NFC_VER_200)
	else if(A_CHANGE_SPARE_ECC_OF(sNAND_IO_DeviceInfo.Feature.MediaType) == A_ECC_SPARE_6BIT)
	{
		pTnftlObj->uiBL1_MediaType |= BL1_MEDIA_TYPE_SPARE_6BIT;
	}
	#endif
...
//yookdh - new one by taeki, defference - copy NAND_IO_deviceInfo.
unsigned int NAND_IO_GetFeatureOfNAND_MTD( NAND_IO_DEVINFO *NAND_IO_deviceInfo, TNFTL_OBJECT_T *pTnftlObj )
{
	unsigned int uiCS_Count = 1;
	memset( &sNAND_IO_DeviceInfo, 0x00, sizeof( sNAND_IO_DeviceInfo ) );
	NAND_IO_Init();
...
#if defined(TCC8925S)|| defined(TCC893X) || defined(TCC897X)
		pTnftlObj->fUseLowerPageOnlyForBL1 = TRUE;
	...
	pTnftlObj->uiCommCycleRegValue = s_CommCycleTime.RegValue;
        #if defined(NAND_FOR_UBOOT)
	memcpy( NAND_IO_deviceInfo, &sNAND_IO_DeviceInfo, sizeof( sNAND_IO_DeviceInfo ) );
        #endif
	return TRUE;
}
...
static unsigned int s_uiPageAddress[4] = {NULL_U32,NULL_U32,NULL_U32,NULL_U32};
...
//yookdh.check - no randomizer... is it ok? now SLC
#ifndef TCC_MTD
	...
	if( nDevInfo->EccType != ECC_TYPE_EMBEDDED )
	{
		...
		{
			if(0)// sNAND_IO_fSuppressMessage == FALSE )
...
#if defined(NAND_FOR_UBOOT)
/* yookdh.check - fault */
unsigned int NAND_IO_ReadSpareArea_MTD( NAND_IO_DEVINFO *nDevInfo, unsigned int uiPageAddress, unsigned int *puiTagBuffer, unsigned int uiTagSize )
{
	NAND_IO_ERROR_T		res = NAND_IO_SUCCESS;
...
static NAND_IO_ERROR_T _NAND_IO_ReadPage(NAND_IO_DEVINFO *nDevInfo,unsigned int uiPageAddress,unsigned int *puiTag, unsigned int uiTagSize)
{
	...
	if( res2 == NAND_IO_CHECK_BOOTAREA_PAGE)
	{
		...
		{
			//yookdh.check
			if( NAND_IO_IsAllValue( puiTag, 0xFF, uiTagSize )) //for erasing block on u-boot
			{
				unsigned int i;
				ND_TRACE("Erased block!!\n");
				for (i=0; i<uiTagSize; i++)
					_ND_TRACE("%2X", puiTag[i]);
				_ND_TRACE("\n");
				res2 = NAND_IO_SUCCESS;
			}
			else
...
/**************************************************************************
*  DESCRIPTION : Evaluate a page if it can be usable when read page operation failed.
*
*  INPUT :
*
*  OUTPUT :
*
*  REMARK : Empty page can be disrupted 
*           and if its error bits didn't exceed ECC capability, it can be usable.
*
**************************************************************************/
unsigned int NAND_IO_IsEmptyPageFlipping(TNFTL_OBJECT_T *pTnftlObj, unsigned int *puiBuffer, unsigned int *puiTag)
{
	NAND_IO_DEVINFO *nDevInfo = &sNAND_IO_DeviceInfo;
	unsigned int res = TRUE;
	unsigned int i, j, uiShiftee, uiAcc;

	/* accumulate error bits for PageBuffer */
	for(i=0, uiAcc=0; i < (pTnftlObj->uiBytesPerPage>>2); i++)
	{
		if((puiBuffer[i]^0xFFFFFFFF) != 0)
		{
			 uiShiftee = puiBuffer[i]^0xFFFFFFFF;
			 for(j=0; (j<BITS_PER_WORD) && (uiShiftee != 0); j++, uiShiftee = uiShiftee >> 1)
			 {
				if((uiShiftee&0x1) != 0)
					uiAcc++;
			 }
	...
	/* accumulate error bits for TagBuffer */
	for(i=0; i < (TNFTL_TAG_BYTE_SIZE>>2); i++)
	{
		if((puiTag[i]^0xFFFFFFFF) != 0)
		{
			 uiShiftee = puiTag[i]^0xFFFFFFFF;
			 for(j=0; (j<BITS_PER_WORD) && (uiShiftee != 0); j++, uiShiftee = uiShiftee >> 1)
			 {
				if((uiShiftee&0x1) != 0)
					uiAcc++;
	...
	/* compare with ECC capability */
	if(uiAcc > NAND_IO_GetECCInfo(nDevInfo->EccType)->ucErrorNum)
	{
		ND_DEBUG("NAND_IO_IsEmptyPageFlipping(%d)\n", uiAcc);
		res = FALSE;
	}
	return res;
}
...
/**************************************************************************
*  DESCRIPTION : To print the page buffer and FTL tag when error occured.
*
*  INPUT :
*
*  OUTPUT :
*
*  REMARK : param should be set before calling this function.
*
**************************************************************************/
void NAND_IO_PrintPage( param_print param )
{
	unsigned int i;
	NAND_IO_DEVINFO		*nDevInfo = &sNAND_IO_DeviceInfo;

	ND_TRACE("################ READ ERROR PAGE DUMP ################\n");
	ND_TRACE("Read Error : 0x%x\n", param.uiErrorCode);
	ND_TRACE("Die : %d\n", param.uiCurrentDieIndex);
	ND_TRACE("Page Address : 0x%x\n", param.uiPageAddress);
	ND_TRACE("######################################################\n");
	
	for( i=0; i<(nDevInfo->Feature.PageSize>>2); i++ )
	{
		if( (i&0x3) == 0 )
		{
			_ND_TRACE("[0x%08X] ", (i<<2));
		}

		_ND_TRACE("%08X ", param.puiPageBuffer[i]);
	...
	for( i=0; i<(param.uiTagSize>>2); i++ )
	{
		_ND_TRACE("%08X ", param.puiTag[i]);
	}
	_ND_TRACE("\n");
}
...
unsigned int NAND_IO_ReadPage( TNFTL_OBJECT_T *pTnftlObj, unsigned int uiPageAddress, unsigned int *puiBuffer, unsigned int *puiTag, unsigned int uiTagSize, unsigned int fNextPageForNextTurn )
{
	...
	{/*
		unsigned int tmp_i;
		unsigned char* tmp_p=(unsigned char*)s_puiDMA_WorkBuffer;
		for(tmp_i=0; tmp_i<nDevInfo->Feature.PageSize; tmp_i++) 
		{
			if((tmp_i%16)==0)
				_ND_TRACE("[page : %d]\n",uiPageAddress );			
			_ND_TRACE("%02X ", tmp_p[tmp_i]);		
		}
		_ND_TRACE("\n");*/
		//ND_TRACE( "MemCpy!!! puiBuffer=0x%x, s_puiDMA_WorkBuffer=0x%x\n" , puiBuffer, s_puiDMA_WorkBuffer );
		NAND_Util_memcpy( (void *)puiBuffer, (const void *)s_puiDMA_WorkBuffer, nDevInfo->Feature.PageSize );
	}

	if( res != NAND_IO_SUCCESS )
	{
		if(NAND_IO_IsEmptyPageFlipping(pTnftlObj, puiBuffer, puiTag) != TRUE)
		{
		    NAND_IO_SetLastError( res );
		    NAND_IO_ResetEccErrorBitCount();
		    return FALSE;
...
*
**************************************************************************/
NAND_IO_ERROR_T NAND_IO_ReadUserSizeData( NAND_IO_DEVINFO *nDevInfo, unsigned short nColumnAddr, unsigned long nReadSize, unsigned char *nReadBuffer )
{
...
unsigned int NAND_IO_EraseBlock( TNFTL_OBJECT_T *pTnftlObj, unsigned int uiBlockAddress )
{
	...
	if( res == NAND_IO_SUCCESS )
	{
		//ND_TRACE("block %d, Page %d\n", uiBlockAddress, uiBlockPageAddr);
		//=============================================
		// Erase Block
		//=============================================
...
void NAND_IO_MarkBad( TNFTL_OBJECT_T *pTnftlObj, unsigned int uiBlockAddress )
{
	...
	unsigned int uiBlockPageAddr = uiBlockAddress << sNAND_IO_DeviceInfo.ShiftPpB;//PageAddressOfBlock( pTnftlObj, uiBlockAddress );
...
void NAND_IO_BusControl( NAND_IO_DEVINFO *nDevInfo )
{
	...
	nMaxBusClk = 1660000;	// frequency of 100Hz unit
		...
		nMaxBusClk = tcc_get_clkctrl( FBUS_IO );
#endif
...
void NAND_IO_SetupECC( unsigned short nEccOnOff, unsigned short nEncDec, unsigned short nEccType, unsigned short nAccessType, unsigned long EccBaseAddr )
{
	...
	else if( nEccOnOff == ECC_ON )
	{
		...
		if( nEncDec == ECC_DECODE )
		{
			...
			{
				s_pECC->ECC_CTRL	|= 	ECC_CTRL_ECC_EN_MLC_ECC6_DECODE;
			}
#if defined(NFC_VER_50) || defined(NFC_VER_100) || defined(NFC_VER_300)
			else if( nEccType == ECC_TYPE_8BIT )
			{
		...
		else if( nEncDec == ECC_ENCODE )
		{
			...
			if( nEccType == ECC_TYPE_4BIT )
				...
				s_pECC->ECC_CTRL	|= 	ECC_CTRL_ECC_EN_MLC_ECC6_ENCODE;
#if defined(NFC_VER_50) || defined(NFC_VER_100) || defined(NFC_VER_300)
			else if( nEccType == ECC_TYPE_8BIT )
...
static NAND_IO_ERROR_T NAND_IO_ReadParam_ONFI(NAND_IO_DEVINFO *pstDevInfo)
{
	unsigned char		pucParam[512];
	NAND_IO_ERROR_T		res;

	//=============================================
	// Check Device and Parameter
	//=============================================
	if((pstDevInfo->IoStatus & NAND_IO_STATUS_ENABLE) == 0)
	{
		res =  NAND_IO_ERROR_DEVICE_IO_IS_NOT_READY;
	}
	else if(pstDevInfo->Feature.DeviceID.Code[0] == MICRON_NAND_MAKER_ID
		|| pstDevInfo->Feature.DeviceID.Code[0] == SPANSION_NAND_MAKER_ID
		|| pstDevInfo->Feature.DeviceID.Code[0] == MACRONIX_NAND_MAKER_ID
		|| pstDevInfo->Feature.DeviceID.Code[0] == WINBOND_NAND_MAKER_ID)
	{
		res = NAND_IO_SUCCESS;
	}
	else
	{
		res = NAND_IO_NOT_ONFI;
	}
	...
	{
		NAND_IO_PreProcess();
		NAND_IO_BusControl( pstDevInfo );
		NAND_IO_SetCommCycleTime();
		NAND_IO_EnableChipSelect( pstDevInfo->ChipNo );
		NAND_BD_Enable_WP_Port();

		if( pstDevInfo->fInterleaveUsable )
			res = NAND_IO_WaitBusyForInterleave( pstDevInfo );

		//ND_TRACE("NFC_IRQ = 0x%x\n", s_pNFC->NFC_IRQ);
		//s_pNFC->NFC_IRQ = NFC_IRQ_READY_FLAG;

		s_pNFC->NFC_CMD = pstDevInfo->CmdMask & 0xECEC;		/* command : 0xEC */
		s_pNFC->NFC_SADDR	= 0x0000;						/* address : 0x00 */

		/* Wait until it is ready */
		NAND_IO_WaitBusy( pstDevInfo->ChipNo );

		/* Read Data */
		res = NAND_IO_ReadUserSizeData( pstDevInfo, 0, sizeof(pucParam), pucParam );
		if( res == NAND_IO_SUCCESS )
		{
			/* implement here for your NAND */
			if((pstDevInfo->Feature.DeviceID.Code[0] == 0x2C)
				&& (pstDevInfo->Feature.DeviceID.Code[1] == 0x64) && (pstDevInfo->Feature.DeviceID.Code[2] == 0x44)
				&& (pstDevInfo->Feature.DeviceID.Code[3] == 0x4B) && (pstDevInfo->Feature.DeviceID.Code[4] == 0xA9))
			{
				/* MT29F64G08CBABA, MT29F128G08CFABA */
				if((pucParam[6] != 0xD8) || (pucParam[7] != 0x01))
				{
					ND_TRACE("Not support this NAND(0x%x). Please contact to Telechips, inc.\n", (unsigned)pucParam[6]+pucParam[7]);
					res = NAND_IO_ERROR_NOT_SUPPORT_NAND;
				}
			}
		}
...
void NAND_IO_GetDevInfo(NAND_IO_DEVINFO *nDevInfo)
{
	memcpy(nDevInfo, &sNAND_IO_DeviceInfo, sizeof(NAND_IO_DEVINFO));
}
