#include <common.h>
#include <power/daudiokk_pfuze100.h>
...
void pfuze100_init(int i2c_ch)
{
	...
	printf( "[PMIC] pfuze100_init PFUZE100 DEVID=0x%x\r\n", data);
	
	pfuze100_write(0x20, 0x1F);
	data = pfuze100_read(0x20);
	printf( "[PMIC] ADDR 0x20 = 0x%x\r\n", data);

    //buffalo+ 
	pfuze100_write(0x66, 0x4A); //requested by kwak ki-tae 
	data = pfuze100_read(0x66);
    printf( "[PMIC] ADDR 0x66 = 0x%x\r\n", data);
	//buffalo-
...
static int pfuze100_output_control(pfuze100_src_id id, unsigned int onoff)
{
	...
	printf( "[PMIC] %s: id:%d, onoff:%d, reg:0x%x, value:0x%x\n", __func__, id, onoff, reg, value);
...
static int pfuze100_buck_set_voltage(pfuze100_src_id id, unsigned int uV)
{
	...
	printf( "[PMIC] %s: id:%d, uV:%d, reg:0x%x, value:0x%x\n", __func__, id, uV, reg, value);
	pfuze100_write(reg, value);
...
{
	printf( "[PMIC] pfuze100_set_voltage : %d\n", mV);
